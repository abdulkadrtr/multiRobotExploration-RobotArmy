...
def exploration(data,width,height,resolution,column,row,originX,originY,choice):
    ...
    if choice == 0:
        ...
        if f == -1:
            ...
            TB1_PATH = path
            TB1_PATHF = f
    #matplotlip ile görselleştirme
    """
    plt.imshow(data, cmap='Greys', origin='lower')
    plt.plot(column,row,'ro') #Robot konumu kırmızı
    new_path = []
    for p in path:
        y = (p[0] - originX) / resolution
        x = (p[1] - originY) / resolution
        new_path.append((x, y))
    new_path = np.array(new_path)
    plt.plot(new_path[:, 1], new_path[:, 0], 'b-') #Yol mavi
    plt.show()
    """
    return
